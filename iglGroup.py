...
def handle_start(message):
    ...
    welcome_message = f"""
    Welcome to 🪬 ILLEGAL SERVER FREEZE DDoS Bot!
 ANY ENQUIRIES DM_ @ILLEGALCHEAT78
...
def handle_attack_command(message):
    ...
    bot.send_message(message.chat.id, "💣 Ready to launch an attack?\n"
                                      ...
                                      "Example: `167.67.25 22452 300` 🔥", parse_mode='Markdown')
...
def process_attack_command(message):
    try:
        ...
        if target_port in blocked_ports:
            ...
            return
        if duration > 300:
            bot.send_message(message.chat.id, "⏳ Maximum duration is 300 seconds.", parse_mode='Markdown')
            return
